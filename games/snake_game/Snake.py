...
class snake:
    ...
    def move(self):
        ...
        for i, c in enumerate(
            self.body
        ):  # checking for the index and cube in self.body
            p = c.pos[:]
            if p in self.turns:
                turns = self.turns[p]
                ...
                if (
                    i == len(self.body) - 1
                ):  # if we are on the last cube we are gonna remove that turn ohterwise next time it will automaticlly turn at that position
                    self.turns.pop(p)
            else:
...
def main():
    ...
    s = snake(
        (255, 0, 0), (10, 10)
    )  # color = red and position = 10,10 bcz we start in the middle
