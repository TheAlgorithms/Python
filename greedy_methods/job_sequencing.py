def job_sequence_with_deadlines(jobs):
    ...
    n = len(jobs)
    max_deadline = max(jobs, key=lambda job: job[1])[1]  # Find the maximum deadline among all jobs
    ...
    return [job_id for job_id in result if job_id != -1]  # Return the sequence of selected jobs
