'''It is highly recommended to visit this page before trying to run the code,
   all the function names then would be self explanatory thereafter
   https://en.wikipedia.org/wiki/Data_Encryption_Standard .
   This class provides various functions for the DES algorithm implementation,
   import this class to use these functions.
'''


class DesAlgo:
    initial_permutation = [58, 50, 42, 34, 26, 18, 10,
                           2, 60, 52, 44, 36, 28, 20,
                           12, 4, 62, 54, 46, 38, 30, 22,
                           14, 6, 64, 56, 48, 40, 32,
                           24, 16, 8, 57, 49, 41, 33, 25,
                           17, 9, 1, 59, 51, 43, 35,
                           27, 19, 11, 3, 61, 53, 45, 37,
                           29, 21, 13, 5, 63, 55, 47, 39,
                           31, 23, 15, 7]
    inverse_permutaion = [40, 8, 48, 16, 56, 24, 64, 32,
                          39, 7, 47, 15, 55, 23, 63,
                          31, 38, 6, 46, 14, 54, 22, 62,
                          30, 37, 5, 45, 13, 53,
                          21, 61, 29, 36, 4, 44, 12, 52,
                          20, 60, 28, 35, 3, 43,
                          11, 51, 19, 59, 27, 34, 2,
                          42, 10, 50, 18, 58, 26, 33, 1,
                          41, 9, 49, 17, 57, 25]
    exp_table = [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8,
                 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
                 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24,
                 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1]
    pc1_table = [57, 49, 41, 33, 25, 17, 9, 1, 58, 50,
                 42, 34, 26, 18, 10, 2, 59, 51, 43, 35,
                 27, 19, 11, 3, 60, 52, 44,
                 36, 63, 55, 47, 39, 31, 23, 15,
                 7, 62, 54, 46, 38, 30, 22, 14, 6, 61,
                 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4]
    shifts = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]
    bit_shuffle = [16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18,
                   31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6,
                   22, 11, 4, 25]
    pc2_table = [14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
                 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13,
                 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45,
    ...
    def initial_permutations(self, msg: str) -> str:
        return self.crank(self.single_string(msg), self.initial_permutation)

    def inverse_permutation(self, msg: str) -> str:
        return self.crank(self.single_string(msg), self.inverse_permutaion)

    def expansion(self, msg: str) -> str:
        return self.crank(self.single_string(msg), self.exp_table)

    def key_to_fifty_six(self, key: str) -> str:
        key = self.single_string(key)
        t = ''
    ...
    def pc1(self, key: str) -> str:
        return self.crank(self.single_string(key), self.pc1_table)

    def shift_left(self, key: str, rnd: int) -> str:
        key = self.single_string(key)
        left = key[0:28]
        right = key[28:]
        i = self.shifts[rnd - 1]
    ...
    def xor(self, s1: str, s2: str) -> str:
        s1 = self.single_string(s1)
        s2 = self.single_string(s2)
        ans = ''
    ...
    def s_box(self, msg: str) -> str:
        ...
        for i in range(0, 48, 6):
            ...
            col = int(temp[1:5], 2)
            code = bin(self.s_boxes[n][row][col])[2:]
            ans += '0' * (4 - len(code)) + code
            n += 1
        return ans
    ...
    def solve(self, msg: str, key: str, rnd: int) -> str:
        ...
        msg = self.initial_permutations(msg)
        print(self.pretty(msg))
