...
# RFC 3526 - Daha Modüler Üstel (MODP) Diffie-Hellman grupları için
# İnternet Anahtar Değişimi (IKE) https://tools.ietf.org/html/rfc3526
...
class DiffieHellman:
    """
    Diffie-Hellman anahtar değişim protokolünü temsil eden sınıf
    ...
    >>> # DH nesnesi kullanarak paylaşılan anahtar oluşturma
    >>> alice_shared = alice.generate_shared_key(bob_public)
    >>> bob_shared = bob.generate_shared_key(alice_public)

    >>> assert alice_shared == bob_shared

    >>> # statik yöntemler kullanarak paylaşılan anahtar oluşturma
    ...
    # Mevcut minimum öneri 2048 bit (grup 14)
    def __init__(self, group: int = 14) -> None:
        if group not in primes:
            raise ValueError("Desteklenmeyen Grup")
    ...
    def is_valid_public_key(self, key: int) -> bool:
        # Diğerinin genel anahtarının NIST SP800-56'ya göre geçerli olup olmadığını kontrol et
        return (
            2 <= key <= self.prime - 2
    ...
    def generate_shared_key(self, other_key_str: str) -> str:
        ...
        if not self.is_valid_public_key(other_key):
            raise ValueError("Geçersiz genel anahtar")
