...
'''
As long as there exists atleast one element in the original array, Priority Scheduling will keep running in loop
'''
while len(processes)>0:
    
    '''
    To deal with more than one process with same priority level, the following array is created
    '''
    ...
    '''
    To counter issues where the processes with the highest priority has not entered the queue yet
    '''
    while len(processes_with_highest_priority)==0:
        
        for process in processes:
            if process.priority == highest_priority and process.arrival_time<=current_time:
    ...
    '''
    In case of more than one process with the same priority level, appyling FCFS algorithm
    '''
    process_to_execute = min(processes_with_highest_priority, key=attrgetter('arrival_time'))
    ...
    process_to_execute.turn_around_time = process_to_execute.waiting_time + process_to_execute.burst_time
...
for process in executed_processes:
    print(str(process.process_name)+'\t\t'+str(process.waiting_time)+'\t\t'+str(process.turn_around_time))
