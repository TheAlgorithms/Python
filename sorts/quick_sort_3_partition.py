...
python3 -m doctest -v quick_sort_3_partition.py

For manual testing run:
python3 quick_sort_3_partition.py
"""

from typing import Optional, List


def quick_sort_3_partition(
    my_list: List[int], left: int = 0, right: Optional[int] = None
) -> List[int]:
    """
    Implementation of quick sort algorithm, 3-Way variation.

    Examples:
    >>> quick_sort_3_partition([0, 5, 3, 2, 2])
    [0, 2, 2, 3, 5]

    >>> quick_sort_3_partition([])
    []

    >>> quick_sort_3_partition([-2, -5, -45])
    [-45, -5, -2]
    """
    if right is None:
        right = len(my_list) - 1
    ...
    while i <= temp_right:
        if my_list[i] < pivot:
            ...
            i += 1
    quick_sort_3_partition(my_list, left, temp_left - 1)
    quick_sort_3_partition(my_list, temp_right + 1, right)
    return my_list
...
if __name__ == "__main__":
    ...
    print(quick_sort_3_partition(unsorted))
