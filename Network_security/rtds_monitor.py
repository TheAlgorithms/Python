...
class FileMonitorHandler(FileSystemEventHandler):
    ...
    def scan_file_with_virustotal(self, file_path):
        ...
        try:
            ...
            print(
                f"🔍 Scanning file: {os.path.basename(file_path)} (SHA256: {sha256[:16]}...)"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"\033[96m📤 File not in database, uploading: {os.path.basename(file_path)}\033[0m"
                )
                upload_result = self._upload_file_to_virustotal(file_path, file_content)
                if upload_result:
                    self.log_callback(
                        f"📤 UPLOADED: {file_path} - Analysis pending", "UPLOADED"
                    )
                else:
                    self.log_callback(
                        f"📁 NEW FILE: {file_path} - Upload failed", "NEW_FILE"
                    )
            else:
        ...
        try:
            ...
            if self.quarantine_dir:
                ...
                quarantine_path = os.path.join(
                    self.quarantine_dir, f"{timestamp}_{file_name}"
                )
...
class IntegratedRTDSMonitor:
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"[!] Warning: Could not get local IP for interface '{self.iface}'. Check interface name or permissions."
            )

        # --- IP Blocking System ---
        self.ip_blocker = IPBlocker(log_callback=self.log_alert)
        print(
            f"[*] IP Blocker initialized - Whitelist entries: {len(self.ip_blocker.whitelist)}"
        )
        ...
        if current_time - self.last_reset >= 1.0:
            ...
            for ip, count in self.packet_counts.items():
                if count > self.ddos_threshold:
                    ...
                    if self.ip_blocker.block_ip(
                        ip, f"DDoS attack - {count} pps", duration=3600
                    ):
                        print(
                            f"\033[93m🚫 Auto-blocked IP: {ip} for DDoS attack\033[0m"
                        )
            ...
            for ip, count in self.syn_counts.items():
                if count > self.syn_threshold:
                    self.total_attacks += 1
                    alert_msg = (
                        f"🚨 SYN Flood from {ip} - Rate: {count} SYN packets/sec"
                    )
                    print(f"\033[91m{alert_msg}\033[0m")
                    ...
                    # Block the attacking IP
                    if self.ip_blocker.block_ip(
                        ip, f"SYN flood - {count} SYN/sec", duration=1800
                    ):
    ...
    def detect_mitm_attack(self, packet):
        ...
        if src_ip in self.arp_table and self.arp_table[src_ip] != src_mac:
            ...
            if self.ip_blocker.block_ip(
                src_ip, f"ARP spoofing - MAC conflict", duration=7200
            ):
    ...
    def setup_file_monitoring(self):
        ...
        try:
            ...
            self.file_observer.schedule(
                self.file_handler, self.monitor_path, recursive=True
            )
...
""")
        ...
        print(
            f"[*] DDoS Threshold: {self.ddos_threshold} pps | SYN Threshold: {self.syn_threshold} pps"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Integrated RTDS - Network & File Threat Detection"
    )

    # Network monitoring arguments
    parser.add_argument(
        "--ddos-threshold", type=int, default=100, help="DDoS packet threshold (pps)"
    )
    parser.add_argument(
        "--syn-threshold", type=int, default=50, help="SYN flood threshold (pps)"
    )
    parser.add_argument(
        "--iface", default="Wi-Fi", help="Network interface (Windows example: Wi-Fi)"
    )
    ...
    parser.add_argument(
        "--quarantine-dir",
        type=str,
        help="Directory to quarantine malicious files (default: delete)",
    )
    parser.add_argument(
        "--no-auto-delete",
        action="store_true",
        help="Disable automatic deletion of malicious files",
    )

    # General arguments
    parser.add_argument(
        "--log", type=str, default="integrated_rtds_alerts.log", help="Log file path"
    )
    ...
    if vt_api_key:
        ...
        user_input = input(
            "Enter directory path to monitor (or press Enter to skip file monitoring): "
        ).strip()
        if user_input:
