...
def depth_first_search(visited: list, graph: dict, start: str) -> list:
    ...
    if start not in visited:
        visited.append(start)  # Puts the first visited letter in the sequence
        for neighbour in graph[start]:
            depth_first_search(
                visited, graph, neighbour
            )  # Uses recursion to check the if statement again. If the if statement is not met, it returns the existing list.
