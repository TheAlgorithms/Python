...
def add_key_to_lexicon(lexicon, curr_string, index, last_match_id):
    """
...
def compress_data(data_bits):
    ...
    while curr_string != "" and curr_string not in lexicon.keys():
        curr_string += "0"
...
def compress(source, destination):
    ...
    compressed = add_file_length(source, compressed)
    write_file_binary(destination, compressed)
