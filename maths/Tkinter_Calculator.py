...
def takeValueInput():
    ...
    try:
        ...
        if len(first_number_entry.get()) == 0 \
            or len(second_number_entry.get()) == 0:
...
minus_button = tk.Button(root, text='-', command=lambda : subtract())
minus_button.pack()

multiply_button = tk.Button(root, text='*', command=lambda : multiply())
multiply_button.pack()

division_button = tk.Button(root, text='/', command=lambda : divide())
