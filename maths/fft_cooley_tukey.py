...
def fft_cooley_tukey(signal: List[complex]) -> List[complex]:
    ...
    result = [0] * n
    for k in range(n // 2):
        t = odd[k] * complex(math.cos(-2 * math.pi * k / n), 
...
if __name__ == "__main__":
    ...
    signal = [math.sin(2 * math.pi * 50 * x) + 0.5 * math.sin(2 * math.pi * 120 * x) 
              for x in t]
    ...
    print(f"Peak frequencies detected: {[f for f, m in zip(frequencies[:len(frequencies)//2], magnitudes[:len(magnitudes)//2]) if m > max(magnitudes[:len(magnitudes)//2]) * 0.1]}")
