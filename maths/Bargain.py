...
for rr in range(1,n+1):
    ...
    if leftIdx==-1:
        ans+=1*right[rr]
        ans%=MOD
#        print('FIRST {}'.format(1*right[rr]))
    elif rr==n:
        nLeft=leftIdx+2
        ans+=left[leftIdx]
        ans%=MOD
#        print('LAST {}'.format(left[leftIdx]))
    else:
        nLeft=leftIdx+2
#        print('rr:{} left[leftIdx]:{} n-rr:{} increment:{}'.format(rr,left[leftIdx],n-rr,left[leftIdx]*(10**(n-rr))+nLeft*right[rr]))
