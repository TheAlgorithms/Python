...
while True:
    try:
        number = int(
            input("Enter the number of sequence you want to generate:").strip()
        )
    except ValueError:
...
print(
    "\n{0}, your requested Fibonacci sequence with {1} terms is: {2}".format(
        me(), number, fibonacci(number)
    )
)
...
print(
    "\n{}Please select an extra operation if you wish"
    '\nInput you reply "no" if you\'re not interested'
