...
class ContourDetector:
    ...
    def detect_contours(self, gray_image: np.ndarray,
                        lower_threshold: int = 50,
                        upper_threshold: int = 150) -> np.ndarray:
        """
        Detects contours in the grayscale image using the Canny edge detector.

        Args:
            gray_image (np.ndarray): The grayscale input image.
            lower_threshold (int): The lower threshold for Canny edge detection
            (default: 50).
            upper_threshold (int): The upper threshold for Canny edge detection
            (default: 150).
    ...
    def save_image_with_contours(self, output_path: str,
                                 contour_image: np.ndarray) -> None:
        """
