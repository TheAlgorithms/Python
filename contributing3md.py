...
class StudentManagementSystem:
    ...
    def add_course(self, name: str, instructor: str, credits: int, 
                   description: str) -> str:
    ...
    def add_grade(self, student_id: str, course_id: str, 
                  grade: float, assignment: str) -> bool:
    ...
    def search_courses(self, query: str) -> List[Course]:
        ...
        for course in self.courses.values():
            if (query in course.name.lower() or 
                ...
                query in course.course_id.lower()):
        ...
        for course_id in student.enrolled_courses:
            ...
            if course:
                ...
                report['courses'].append({
                    ...
                    'attendance': f"{round(attendance_pct, 2)}%" if attendance_pct else 'N/A',
    ...
    def get_course_statistics(self, course_id: str) -> Optional[Dict]:
        ...
        stats = {
            ...
            'average_grade': round(sum(all_grades) / len(all_grades), 2) if all_grades else 'N/A',
            'average_attendance': round(sum(all_attendance) / len(all_attendance), 2) if all_attendance else 'N/A'
        }
        ...
        if choice == '1':
            ...
            if results:
                ...
                for course in results:
                    print(f"  ID: {course.course_id} | Name: {course.name} | Instructor: {course.instructor}")
            else:
            ...
            if sms.students:
                for student in sms.students.values():
                    print(f"ID: {student.student_id} | Name: {student.name} | Courses: {len(student.enrolled_courses)}")
            else:
            ...
            if sms.courses:
                for course in sms.courses.values():
                    print(f"ID: {course.course_id} | Name: {course.name} | Students: {len(course.enrolled_students)}")
            else:
            ...
            print("\n--- Delete Student ---")
            student_id = input("Student ID: ")
            confirm = input(f"Are you sure you want to delete student {student_id}? (yes/no): ")
            ...
            print("\n--- Delete Course ---")
            course_id = input("Course ID: ")
            confirm = input(f"Are you sure you want to delete course {course_id}? (yes/no): ")
