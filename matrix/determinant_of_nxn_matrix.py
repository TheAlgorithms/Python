"""
    ...
    15.0
    >>> determinant_of_nxn_matrix([[1, 2, 3, 7], [4, 5, 6, 6], [7, 8, 1, 5], [1, 2, 3, 4]])
    -72.0
    >>> determinant_of_nxn_matrix([[1, 2, 3, 7, 13], [4, 5, 6, 6, 90], [7, 8, 1, 5, 76], [1, 2, 3, 4, 12], [9, 6, 3, 7, 4]])
    19848.0
    >>> determinant_of_nxn_matrix([[1, 2,  3, 7,  13, 23], [4, 44, 6, 6,  90, 12], [7, 8,  1, 5,  6,  98], [1, 2,  3, 4,  12, 4], [9, 6,  3, 7,  4,  9], [2, 47, 8, 91, 36, 9]])
    -20981553.999999993
"""
...
def determinant_of_nxn_matrix(matrix: list[list[float]]) -> float:
    ...
    for k in range(1, (2*(N-1)) + 1):
        for i in range(c1, N):
            for j in range(0, N):
                if (matrix[c1 - 1][c2]) != 0:
                    matrix_copy[i][j] = (matrix[i][j] - ((matrix[m][n]) * ((matrix[c2][j]) / (matrix[c1 - 1][c2]))))
            m = m + 1
