...
def parentheses(operand, operator) -> int:
    ...
    minimum_numbers = [[None for x in range(number_of_operands)] for x in range(number_of_operands)]
    maximum_numbers = [[None for x in range(number_of_operands)] for x in range(number_of_operands)]
    ...
    for s in range(1, number_of_operands):
        for i in range(0, number_of_operands - s):
            j = i + s
            minimum_numbers[i][j], maximum_numbers[i][j] = MinandMax(maximum_numbers, minimum_numbers, i, j, operator)
