...
def floyd_warshall(adjacency_matrix: list[list[int]], n:int) -> list[list[int]]:
    ...
    >>> floyd_warshall([[-1, 2, -1, -1], [1, -1, 3, -1], [-1, -1, -1, -1], [3, 5, 4, -1]],4)
    ...
    for intermediate_vertex in range(n):
        for vertex1 in range(n):
            for vertex2 in range(n):
                if(shortest_distance[vertex1][intermediate_vertex]!=sys.maxsize and shortest_distance[intermediate_vertex][vertex2] != sys.maxsize):
                    shortest_distance[vertex1][vertex2]=min(shortest_distance[vertex1][vertex2],shortest_distance[vertex1][intermediate_vertex]+shortest_distance[intermediate_vertex][vertex2])
...
if __name__ == "__main__":
    ...
    # directed graph
    adjacency_matrix[0][1] = 2  #vertex 1 can be reached from vertex 0 at cost/distance of 2
    adjacency_matrix[1][0] = 1  #vertex 0 can be reached from vertex 1 at cost/distance of 1
    adjacency_matrix[1][2] = 3  #vertex 2 can be reached from vertex 1 at cost/distance of 3
    adjacency_matrix[3][0] = 3  #vertex 0 can be reached from vertex 3 at cost/distance of 3
    adjacency_matrix[3][1] = 5  #vertex 1 can be reached from vertex 3 at cost/distance of 5
    adjacency_matrix[3][2] = 4  #vertex 2 can be reached from vertex 3 at cost/distance of 4
