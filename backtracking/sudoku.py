...
This algorithm shows a big difference in EASY sudokus(solves without step 3)
but only a slight difference in HARD sudokus
"""
...
options = [
    [[i + 1 for i in range(Length)] for i in range(Length)] for i in range(Length)
]
...
def backtrack_init(grid):
    # temporary grid to perform backtrack
    global temp_grid
    for i in range(Length):
        for j in range(Length):
