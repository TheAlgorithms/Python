...
def solver(i=0, j=0, n=0):
    ...
    for z in range(1, 10):
        ...
        if z not in row[x] and z not in col[y] and z not in table[temp]:
            grid[x][y] = z
            row[x].add(z)  # new value is added to that specific row
            col[y].add(z)  # new value is added to that specific column
            table[temp].add(z)  # new value is added to that mini table

            if solver(x, y, n + 1):
                return True

            grid[x][y] = 0
            row[x].remove(z)  # new value is removed from that specific row
            col[y].remove(z)  # new value is removed from that specific column
            table[temp].remove(
                z
            )  # the new value is removed from that specific mini table
