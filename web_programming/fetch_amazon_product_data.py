...
def get_product_info(product="laptop") -> None:
    # function that will take the product as input and return the product details as output
    # in the form of a csv file,if no input is given,it will fetch the details of laptop by default
    url = (
    ...
    }  # header that will indicate to the destination server that the request is coming from
    # a genuine human being and not a bot
    ...
    ):  # for loop to parse through each entry and store them in the dataframe along with try....
        # except block for handling exceptions that may arise
        try:
