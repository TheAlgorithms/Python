...
def read_lines_strip_return(file_name: str, split: str = None, index: int = None) -> list:
    """
    ...
    - If the index is specified then after splitting the line corresponding index from the line is returned
    - Returns list of lines from the file file_name matching the split and index conditions
    """
    try:
        with open(file_name) as readfile:
...
def append_line_to_file(file_name: str, line: str) -> None:
    ...
    - Opens the file in append mode and appends the line at the ending of file file_name.
    """    
    touch_file(file_name)
