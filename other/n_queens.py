...
def nqueens(board_width):
    ...
    while True:
        ...
        for review_index in range(0, current_row):
            ...
            if (
                board[current_row] == board[review_index]
                or (left >= 0 and left == board[current_row])
                or (right < board_width and right == board[current_row])
            ):
                conflict = True
                break
...
if __name__ == "__main__":
    ...
    if default_width == 8:
        print(
            "Running algorithm with board size of 8. Specify an alternative Chess board size for N-Queens as a command line argument."
        )
