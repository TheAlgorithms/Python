...
class BezierCurve:
    ...
    def __init__(self, list_of_points: List[Tuple]):
        ...
        self.degree = len(list_of_points) - 1 #degree of the curve produced will be n - 1
    ...
    def basis_function(self, t: float) -> List[float]:
        ...
        for i in range(len(self.list_of_points)):
            output_values.append(sp.comb(self.degree, i) * ((1 - t) ** (self.degree - i)) * (t ** i)) #basis function for each i

        assert round(sum(output_values),5) == 1 #the basis must sum up to 1 for it to produce a valid bezier curve.
    ...
    def bezier_curve_function(self, t: float) -> Tuple[float, float]:
        ...
        x = 0.0
        y = 0.0
        for i in range(len(self.list_of_points)): #summing up the product of ith basis function and ith point, for all points
