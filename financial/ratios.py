"""
Financial ratios are quantitative metrics used to analyze and assess the relationships between
different financial elements in a company's financial statements, providing insights into its performance,
...
The quick ratio, also known as the acid-test ratio, is a financial ratio that measures a company's ability
to cover its short-term liabilities with its most liquid assets, excluding inventory.
It is calculated by dividing the sum of cash, marketable securities, and accounts receivable by the
total current liabilities. The quick ratio provides a more conservative assessment of a company's liquidity
compared to the current ratio, as it excludes inventory, which may not be as readily convertible
to cash in the short term. A higher quick ratio indicates a stronger ability to meet short-term obligations.
"""


def current_ratio(current_assets: int, current_liabilities) -> float:
    """
    >>> current_ratio(100, 100)
    1.0
