...
class Trie:
    ...
    def _pattern_search_helper(self, node: TrieNode, current_word: str, 
                             pattern: str, words: List[str]) -> None:
        ...
        if char == '*':
            ...
            for child_char, child_node in node.children.items():
                self._pattern_search_helper(child_node, current_word + child_char, 
                                         pattern, words)
        elif char == '?':
            # Match any single character
            for child_char, child_node in node.children.items():
                self._pattern_search_helper(child_node, current_word + child_char, 
                                         remaining_pattern, words)
        else:
            # Match exact character
            if char in node.children:
                self._pattern_search_helper(node.children[char], current_word + char, 
                                         remaining_pattern, words)
