...
class SparseTable:
    ...
    def min_query(self,l ,r):   # Range minimum Query  :  [l,r)
        length = r-l+1
        k = int(log(length, 2))
        ans = min(self.arr[self.stable[l][k]], self.arr[self.stable[l + length - (1 << k)][k]])
        return ans              # return the minimum number in the range

if __name__ == '__main__':

    arr = [1,2,3,4,5,6,7,8,9]
    s_obj = SparseTable(arr)
    print(s_obj.min_query(2,5))
    print(s_obj.min_query(1,8))
