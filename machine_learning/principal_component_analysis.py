...
data much easier and faster for machine learning algorithms without extraneous variables to process.
"""
...
def center(data: np.ndarray) -> np.ndarray:
    ...
    :data   : a numpy array has shape of (m, n) whereas m is the number of observations and n is the number of features
    :return : a numpy array has shape of (m, n) whereas m is the number of observations and n is the number of features
...
def pca(data: np.ndarray, n_components=2) -> np.ndarray:
    ...
    transformed = X.dot(vec[:, :n_components])      # multiplying data with k sorted eigenvectors
