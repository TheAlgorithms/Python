...
def get_XT() : 
	"""
	Placeholder code to generate the training data, X, Y. (Y is also referred to as T in the code / docs).
	"""
	X = [	[0,0],
		[1,0],
		[2,0],
...
def get_X_by_T(X, T):
	...
	# We could use better numpy operations, but for learning purposes the code needn't be over-complex.
	# We have two classes. 
...
def main() : 
	...
	print ("\n\nStep 2: Get X by classes in T, i.e. all X points of class 1 as X_1, all points of class 2 as X_2.")
	X_1, X_2 = get_X_by_T(X, T)


	print ("\n\nStep 3: Get means of X by class. i.e. m1_mat and m2_mat as means of data X_1 and X_2 respectively.")
	...
	prediction_y = np.matmul(np.transpose(w), point).reshape(1,)
