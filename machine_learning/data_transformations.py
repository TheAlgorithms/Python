"""
min_max_normalization Wikipedia: https://en.wikipedia.org/wiki/min_max_normalization
min_max_normalization is the process of converting numerical data to a standard range of values.
This range is typically between [0, 1] or [-1, 1]. The equation for min_max_normalization is
...
maximum value within the column or list of data. min_max_normalization is used to speed up the
...
also known as z-score min_max_normalization. The equation for standardization is
...
Choosing between min_max_normalization & Standardization is more of an art of a science, but it
...
Additionally, a few rules of thumb are:
    ...
    2. non-gaussian (non-normal) distributions work better with min_max_normalization
    3. If a column or list of values has extreme values / outliers, use standardization
"""
from statistics import mean, stdev


def min_max_normalization(data: list, ndigits: int = 3) -> list:
    """
    ...
    @examples:
    >>> min_max_normalization([2, 7, 10, 20, 30, 50])
    [0.0, 0.104, 0.167, 0.375, 0.583, 1.0]
    >>> min_max_normalization([5, 10, 15, 20, 25])
...
def mean_normalization(data: list, ndigits: int=3) -> list:
    """
    Returns a mean normalized list of values
    @params: data, a list of values to normalize
    @returns: a list of normalized values (rounded to ndigits decimal places)
    """
    # variables for calculation
    ...
    x_max = max(data)
    # normalize data
    return [round((x - avg) / (x_max - x_min), ndigits) for x in data]
