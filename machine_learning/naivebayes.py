...
def cond_prob(X_train, Y_train, feature_col, feature_val, target):
    ...
    numerator = np.sum(
        X_train[:, feature_col] == feature_val
    )  # Sum of all the values that have X corresponding to Y
...
def predict(X_train, Y_train, X_test):
    ...
    pred = np.argmax(
        post_prob
    )  # returns the indices of max posterior probability ; basically the class with max probability, hence the predicted class
