...
def main():
    ...
    X = ["Sepal.Length","Sepal.Width","Petal.Length","Petal.Width"]#X is the inputs
    Y  = ["Species"]#Y is the outputs
    
    # Splitting data into train and test
    Xtrain,Xtest,ytrain,ytest = train_test_split(iris[X],iris[Y],test_size=0.3, random_state=0)#Test_size:Out of 100 % data we split 30 % for testing  & random_state : controls the shuffling process
    ...
    print(f"Cross Val Accuracy:\t {np.mean(cross_val_score(MNB, Xtrain, ytrain.values.ravel(), cv=10)) }")
