...
def train_ngram_model(N):
    ...
    log_probabilities = {
        ngram: math.log(count / total_ngrams) for ngram, count in ngram_counts.items()
    }
...
def calculate_probability(sentence, N):
    ...
    sentence_ngrams = [
        tuple(sentence_words[i : i + N]) for i in range(len(sentence_words) - N + 1)
    ]
