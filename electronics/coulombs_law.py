...
def couloumbs_law(force: float, charge1: float, charge2: float, distance: float):
    ...
    if force == 0:
        return {"force": COULOMBS_CONSTANT * abs(charge1 * charge2) / (distance**2)}
    elif charge1 == 0:
        return {"charge1": abs(force) * (distance**2) / (COULOMBS_CONSTANT * charge2)}
    elif charge2 == 0:
        return {"charge2": abs(force) * (distance**2) / (COULOMBS_CONSTANT * charge1)}
    elif distance == 0:
        return {"distance" : (COULOMBS_CONSTANT * charge1 * charge2 / abs(force))**0.5}
