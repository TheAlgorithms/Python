...
def transient_resp_RL(resistance:float, inductance:float, voltage:float, current:float, time:float) -> tuple:
    ...
    if resistance <= 0:
        ...
        raise ValueError("Voltage and current both cannot be zero please give anyone parameter to calculate")
    else:
        time_const = inductance / resistance
        if (current == 0):
            result_current = (voltage / resistance)*(1 - pow(e,(-1*(time/time_const))))
