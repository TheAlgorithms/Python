...
def is_pandigital():
    ...
    for i in range(1, 10000):
        ...
        while len(num1) < 9:
            ...
            num1 += str(
                i * num2
            )  # concatinating the multiples until num1 has 9 digits.

            num2 += 1

        if (
            (len(num1) == 9) and (len(set(num1)) == 9) and ("0" not in num1)
        ):  # to check if all 1-9 digits are present and 0 is not in the number.
