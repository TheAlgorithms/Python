...
class Point:
	...
	def isZero(self): return self.x == 0 and self.y == 0

	def minus(self, p): return Point(p.x - self.x, p.y - self.y)

	def __str__(self): return f"( {self.x}, {self.y})"

def side(p1, p2):  return ( abs(p2.x - p1.x) ** 2.0 + abs(p2.y - p1.y) ** 2.0 ) ** 0.5
def crossproduct(p1, p2): return p1.x * p2.y - p1.y * p2.x
