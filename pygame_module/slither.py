...
After installing the python. Install the pygame module using the following command: In CLI type:
pip install pygame
#SNAKE GAME --- The snake moves in the box. Make sure it doesn't cross the boundaries.
It's size i.e the length of the snake increases as it eats the food. Navigate the snake using the arrow keys of the keyboard.
To play the game: USE "python slither.py in the CLI for windows from the location of the program. After saving it locally in the pc'''
...
def snake(blockSize, snakelist):
    ...
    for size in snakelist:
        pygame.draw.rect(
            gameDisplay, black, [size[0] + 5, size[1], blockSize, blockSize], 2
        )
