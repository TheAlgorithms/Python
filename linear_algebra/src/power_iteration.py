...
def power_iteration(
    ...
    while not convergence:
        ...
        w: float = np.dot(input_matrix, vector)
        # Normalize the resulting output vector.
        vector = w / np.linalg.norm(w)
        # Find rayleigh quotient
        # (faster than usual b/c we know vector is normalized already)
        lamda: int = np.dot(vector.T, np.dot(input_matrix, vector))
...
def test_power_iteration() -> None:
    ...
    """
    # Our implementation.
    input_matrix: np.array = np.array([[41, 4, 20], [4, 26, 30], [20, 30, 50]])
    vector: np.array = np.array([41, 4, 20])
    eigen_value, eigen_vector = power_iteration(
        input_matrix, vector)  # type: float, float
