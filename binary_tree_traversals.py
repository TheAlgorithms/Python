...
#Initializing New Node for Dynamic Listing
class TreeNode:
...
def build_tree():
    print("\n********Press n/N to stop entering at any point of time********\n")
    check = input("Enter the value of the root node: ").strip().lower() or "n" or "N"
    if check == "n" or "N":
        return None
    ...
    while not q.empty():
        ...
        check = input(msg).strip().lower() or "n" or "N"
        if check == "n" or "N" :
            return tree_node
...
def post_order_iter(node: TreeNode) -> None:
    ...
    4 5 2 6 7 3 1 
    """
    # Popping elements if not in List
    if not isinstance(node, TreeNode) or not node:
        return
    stack1, stack2 = [], []
    n = node
