...
class InputError(Exception):
    def __init__(self, value: type, expected_value: type) -> None:
        self.value = value
...
def input_verification(dictionary: dict) -> None:
    """
    Check that the value provided are the one expected. If not, raise the InputError
    >>> input_verification({int: int})

    >>> input_verification({str: int})
    nasch.InputError: Error! It works with <class 'int'> and not <class 'str'>
    """
...
def get_distance(highway_now: list, car_index: int) -> int:
    ...
    4
    """
    input_verification({type(highway_now): list, type(car_index): int})
...
def update(highway_now: list, PROBABILITY: int, MAX_SPEED: int) -> list:
    """
    ...
    """
    input_verification(
        {type(highway_now): list, type(PROBABILITY): int, type(MAX_SPEED): int}
    )
...
def simulate(
    highway: list, NUMBER_OF_UPDATE: int, PROBABILITY: int, MAX_SPEED: int
) -> list:
    """
